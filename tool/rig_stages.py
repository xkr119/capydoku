...
SPECS = [
    ...
    # 청소년·성인·어른은 눈을 부풀렸다(tool/bigger_eyes.py) — 눈꺼풀도
    # 같이 커져야 깜빡일 때 눈이 다 안 덮인다.
    Spec('s3', 332, 120, 348, 312, [(272, 210), (388, 210)], 17,
         (330, 300, 55, 34),
    ...
    Spec('s4', 330, 150, 350, 300, [(245, 115), (410, 115)], 18,
         (330, 225, 65, 38),
         [(162, 441, 55, 93), (496, 441, 56, 93)], [(186, 359), (472, 359)], [221, 414]),
    # 어른 — 주둥이가 크고 회색이라 턱 타원도 크다.
    Spec('s5', 335, 155, 320, 272, [(240, 60), (425, 60)], 15,
         (322, 190, 75, 45),
         [(133, 455, 61, 116), (525, 455, 60, 116)], [(160, 372), (499, 372)], [213, 447]),
    # 배우자 — 어른보다 작고 얼굴이 둥글다. 성인과 비슷한 몸매.
    # **짝꿍은 팔을 나누지 않는다.**
    #
    # 이 렌더는 앞발을 배 쪽으로 바짝 모으고 있어서, 도려낼 자리가 배 한가운데
    # 까지 파고든다. 좁게 자르면 발톱이 몸통에 남고(팔을 들면 배에 발톱만 둥둥
    # 떴다), 넓게 자르면 메울 털을 어디서도 못 가져온다 — 옆은 9px밖에 안 남아
    # 헤링본 무늬가 깔리고, 위는 바로 턱이라 얼굴이 배로 내려왔다.
    #
    # 팔을 안 나누면 먹을 때 앞발을 못 들지만, **아무것도 망가지지 않는다.**
    # 아기(s1)도 같은 이유로 안 나눈다. 고칠 수 없는 걸 어설프게 고쳐 놓는
    # 것보다 낫다(사용자 판단: "잘 안 되면 그냥 팔은 그냥 둬").
...
# 목 기둥을 자름선 위로 얼마나 세울까(머리 반지름 대비).
#
# **정수리까지 세우면 안 된다.** 예전엔 이미지 꼭대기까지 남겼는데, 고개를
# 숙이면 머리 조각이 내려가면서 그 기둥의 꼭대기가 머리 **위로 드러났다**
# — 머리 윤곽이 하나 더 있는 것처럼 보였다(아기가 당근 먹을 때 특히,
# 머리가 몸의 절반이라 조금만 움직여도 위가 비었다).
#
# 기둥이 필요한 이유는 고개를 돌리거나 젖힐 때 **목 언저리**가 비지 않게
# 하려는 것뿐이다. 자름선 바로 위 한 뼘이면 충분하다.
NECK_RISE_K = 0.55


def neck_column(size, cut_y, spec):
    """자름선 위쪽만 목 굵기로 **조금** 남기는 마스크. 아래는 손대지 않는다.
    ...
    rise = spec.head_half * NECK_RISE_K
    for y in range(H):
        d.line([(0, y), (W, y)], fill=255)          # 기본은 전부 남긴다
    for x in range(W):
        top = int(cut_y(x))
        # 목 안쪽 기둥은 자름선 위로 한 뼘만 남기고, 그 위는 지운다.
        keep_to = int(top - rise) if abs(x - spec.head_cx) <= half else top
        d.line([(x, 0), (x, keep_to)], fill=0)
