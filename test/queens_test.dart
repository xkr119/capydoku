...
void main() {
  group('QueensGenerator — 상품 품질 보증', () {
    ...
    test('골든: 7×7 시드 1은 영원히 이 배치여야 한다', () {
      ...
      //
      // 2026-08-20에 **딱 한 번 의도적으로** 바꿨다: 내부 시드가
      // `seed*1000+attempt`라 시도가 1000번을 넘으면 이웃 시드와 겹쳤고,
      // 그 탓에 한 레벨의 후보 네 개가 전부 같은 퍼즐이 되었다.
      // 미출시 상태였기에 고칠 수 있었다. 이제부터는 고정이다.
      final p = QueensGenerator.generate(n: 7, seed: 1)!;
      expect(p.solution, [4, 6, 3, 1, 5, 2, 0]);
    });
  });
}
